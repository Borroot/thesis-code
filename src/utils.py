    ...
    if act_attr_to_keep == default_act_attr_to_keep:
        ...
        for el in act_attr_to_keep:
            if grid2openv.action_space.supports_type(el):
                ...
                warnings.warn(
                    f"attribute {el} cannot be processed by the allowed "
...
def save_used_attribute(
    ...
    if save_path is not None:
        ...
        with open(
            os.path.join(my_path, "obs_attr_to_keep.json"), encoding="utf-8", mode="w"
        ) as f:
            json.dump(fp=f, obj=obs_attr_to_keep)
        with open(
            os.path.join(my_path, "act_attr_to_keep.json"), encoding="utf-8", mode="w"
        ) as f:
...
class SB3Agent(GymAgent):
    ...
    def load(self):
        ...
        self.nn_model = self._nn_type.load(path_load, custom_objects=custom_objects)
...
def save_log_gif(path_log, res, gif_name=None):
    ...
    for _, chron_name, cum_reward, nb_time_step, max_ts in res:
        ...
        ep_replay.replay_episode(
            episode_id=chron_name, gif_name=gif_name, display=False
        )
...
if __name__ == "__main__":
    ...
    env_gym.observation_space = BoxGymObsSpace(
        env.observation_space, attr_to_keep=obs_attr_to_keep
    )
    env_gym.action_space.close()
    env_gym.action_space = BoxGymActSpace(
        env.action_space, attr_to_keep=act_attr_to_keep
    )

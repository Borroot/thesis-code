...
from grid2op.gym_compat.utils import ALL_ATTR_CONT, GYM_VERSION
from grid2op.Observation import BaseObservation
from gymnasium.spaces import Box
from packaging import version

from utils import remove_non_usable_attr
...
class Env_RLLIB(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.env_gym.observation_space = BoxGymObsSpace(
            self.env_glop.observation_space, attr_to_keep=obs_attr_to_keep
        )
        ...
        self.env_gym.action_space = BoxGymActSpace(
            self.env_glop.action_space, attr_to_keep=act_attr_to_keep
        )

        # 4. specific to rllib
        self.action_space = Box(
            low=self.env_gym.action_space.low,
            high=self.env_gym.action_space.high,
            shape=self.env_gym.action_space.shape,
        )
        self.observation_space = Box(
            low=self.env_gym.observation_space.low,

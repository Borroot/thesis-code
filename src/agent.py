...
class GymAgent(BaseAgent):
    ...
    def __init__(
        ...
        *,  # to prevent positional argument
        ...
        self._has_heuristic: bool = False  # TODO remove all heuristic code
        self.gymenv = gymenv
        self._action_list: Optional[List] = None

        if _check_none_set and (nn_path is None and nn_kwargs is None):
            raise RuntimeError(
                "Impossible to build a GymAgent without providing at "
    ...
    def clean_heuristic_actions(
        self, observation: BaseObservation, reward: float, done: bool
    ) -> None:
    ...
    def act(
        ...
        if self._has_heuristic:
            if not self._action_list:
                ...
                self._action_list = self.gymenv.heuristic_actions(
